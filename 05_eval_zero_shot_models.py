...
# Ground truth: 後16筆
y_true = hi_df["MA50"].values[N_CONTEXT:N_CONTEXT+N_HORIZON]
...
pred_tfm = read_pred("TimesFM")
pred_chr = read_pred("Chronos")
...
plt.plot(x_full, hi_df["MA50"].values, label="Ground Truth (MA50)", linewidth=3, color="black")
...
plt.title("F05 | 48-context + 16-forecast (MA50)")
plt.xlabel("Flight")
plt.ylabel("CV (MA50)")
plt.legend()
